...
text(2,1.1,'MP');
grid
xlabel('Valor actual de L2')
ylabel('pertenencia')


x=linspace(-3*A,3*A,30);
y=linspace(-3*A,3*A,30);
...
for i=1:length(x);
    for j=1:length(y);
        %superficie sugeno 
        zs(i,j)=fmrcL2_takagisugeno(x(i),y(j));
    end
end
...
colormap(gray.^2/2)
xlabel('valor deseado de L2')
ylabel('Valor real de L2')
zlabel('salida del controlador de ajuste')
title('controlador de ajuste para L2')
